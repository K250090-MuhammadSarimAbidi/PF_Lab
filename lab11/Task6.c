...
void displayAll(struct Item *p, int count) {
...
void filterByCategory(struct Item *p, int count, const char *category) {
    int found = 0;
...
void filterByMinQuantity(struct Item *p, int count, int minQty) {
    int found = 0;
...
int main() {
    ...
    filterByCategory(ptr, n, cat);

    int q;
    printf("\nEnter minimum quantity: ");
    scanf("%d", &q);
    printf("FILTER: QUANTITY >= %d \n", q);
    filterByMinQuantity(ptr, n, q);
